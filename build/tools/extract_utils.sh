...
function setup_vendor() {
    ...
    export LAZY_ROOT="$3"
    if [ ! -d "$LAZY_ROOT" ]; then
        echo "\$LAZY_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$LAZY_ROOT/$OUTDIR" ]; then
        mkdir -p "$LAZY_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$LAZY_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$LAZY_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$LAZY_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$LAZY_ROOT"/vendor/lazy/build/tools/smali/baksmali.jar
        export SMALIJAR="$LAZY_ROOT"/vendor/lazy/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$LAZY_ROOT"/vendor/lazy/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$LAZY_ROOT"/vendor/lazy/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ ! -f "$LAZY_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$LAZY_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$LAZY_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$LAZY_ROOT"/vendor/lazy/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$LAZY_ROOT"/"$OUTDIR"/radio
