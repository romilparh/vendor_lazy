...
func init() {
	android.RegisterModuleType("lazy_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = lazyExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = lazyExpandVariables(ctx, dep_file)
	...
	cmd := lazyExpandVariables(ctx, String(g.properties.Cmd))
